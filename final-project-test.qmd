...
player |>
  group_by(nameZone) |>
  summarise(prop = mean(isShotMade))
```
...
```{r}
stats <-
  bref_players_stats(seasons = 2024,
                     tables = "per_game")
stats <-
  stats |>
  select(- c("slugSeason",
...
server <- function(input, output, session) {
  ...
  # model <-
  #   reactive({
  #     df <- glm(isShotMade ~ distanceShot + numberPeriod, data = player_react(), family = "binomial")
  #     grid <- 
  #       df |>
  #       data_grid(
